...
public class Grille implements Serializable {
    ...
    public Grille(Points[][] points) {
        this.lesPoints = points;
    }
    ...
    public int getPoint(int i, int j) {
        //Si le pions existe
        if (lesPoints[i][j] != null)
            return lesPoints[i][j].getJoueur();
        else
            return -1;
    }
    ...
    public boolean addPoint(int colonne, int joueur) {
        for (int i = 5; i >= 0; i--) {
            ...
            if (lesPoints[i][colonne].getJoueur() == -1) {
                lesPoints[i][colonne].setJoueur(joueur);
                return true;
            }
        }
    ...
    boolean horiozontale (int joueur) {
        ...
        for (int i = 0; i < 6; i++) {
            ...
            for (int j = 0; j < 7; j++) {
                ...
                if(lesPoints[i][j].getJoueur() == joueur)
                    pions++;
                else
                    pions = 0;
    ...
    //fonction qui vérifie la victoire à la verticale
